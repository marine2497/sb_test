...
void time_init(void)
{
    uint32_t systick_freq = 72000000  ;
    time.msPeriod = 0;
...
uint64_t time_nowUs(void)
{
	uint64_t v;
    __set_PRIMASK(1);
     v = time.msPeriod * (uint64_t)1000 + (SysTick->LOAD - SysTick->VAL) / time.ticksPerUs;
    __set_PRIMASK(0);
    return v;
}
...
uint32_t time_nowMs(void)
{
	uint32_t v;
    __set_PRIMASK(1);
     v = time.msPeriod + (SysTick->LOAD - SysTick->VAL) / time.ticksPerMs;
    __set_PRIMASK(0);
    return v;
}
